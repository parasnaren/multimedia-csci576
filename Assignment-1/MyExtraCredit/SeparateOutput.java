...
public class SeparateOutput {
    ...
    public BufferedImage getCopyOfImage(BufferedImage out, BufferedImage originalImage) {
	...
	public void showIms(String[] args) {
        ...
        previousImg = new BufferedImage(SIZE, SIZE, BufferedImage.TYPE_INT_RGB);
        nextImg = new BufferedImage(SIZE, SIZE, BufferedImage.TYPE_INT_RGB);
		...
		mainImgTimer.scheduleAtFixedRate(new TimerTask() {
			public void run() {
                Thread.currentThread().setName("Main Image");
                ...
                subImage = mainImg.getSubimage(0, 0, SIZE, SIZE);
                previousImg.createGraphics().drawImage(subImage, 0, 0, null);

                // Initialise mainImg to nextImg
                subImage = nextImg.getSubimage(0, 0, SIZE, SIZE);
                mainImg.createGraphics().drawImage(subImage, 0, 0, null);
		...
		newImgTimer.scheduleAtFixedRate(new TimerTask() {
			public void run() {
                Thread.currentThread().setName("Output Image");
				Graphics g = newImg.createGraphics();
                
                // Copy over mainImg to antiAliasedImage object
                subImage = mainImg.getSubimage(0, 0, SIZE, SIZE);
                BufferedImage antiAliasedImage = new BufferedImage(subImage.getWidth(), subImage.getHeight(), subImage.getType());
                antiAliasedImage.createGraphics().drawImage(subImage, 0, 0, null);
	...
	public static void main(String[] args) {
		SeparateOutput ren = new SeparateOutput();
		ren.showIms(args);
	}
